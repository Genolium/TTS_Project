...
async def local_main():
    while True:
        ...
        print("\nВведите нужный полный путь к нужному вам файлу или папке")
        print("Пример: C:\\Users\\User\\Downloads\\text.pdf или C:\\Users\\User\\Downloads")
        f = input()
        print("Введите номер голоса (baya - 1, aidar - 2, kseniya - 3, xenia - 4)")
        v = int(input())
        if(os.path.exists(f) and v in range(1,5)):
            if os.path.isfile(f) and pathlib.Path(f).suffix in ['.txt','.pdf'] :
                ...
                print("Обрабатываем папку "+f)
                voice = voices[v]
                for filename in os.listdir(f):
                    tempfile = os.path.join(f, filename)
                    if os.path.isfile(tempfile) and pathlib.Path(tempfile).suffix in ['.txt','.pdf']:
                        name = str(datetime.now().timestamp())
                        if not os.path.exists('output'):
                            os.mkdir('output')
                        wav = "output\\" + name + ".wav"
                        mp3 = "output\\" + name + ".mp3"
                        if pathlib.Path(tempfile).suffix == '.txt':
                            start_time = time.time()
                            with open(tempfile, 'r', encoding='utf-8') as file:
                                txt_content_str = file.read()
                            await va_speak(txt_content_str, wav, voice)
                            end_time = time.time()
                            print(Fore.YELLOW + f"Файл {tempfile} длиной в {len(txt_content_str)} символ(-ов) (сохранен в {mp3}) обработан в звук за: {end_time - start_time:.2f} с.")
                            sound = AudioSegment.from_wav(wav)
                            sound.export(mp3, format="wav")
                        else:
                            start_time = time.time()
                            ...
                            await va_speak(text, wav, voice)
                            end_time = time.time()
                            print(Fore.YELLOW + f"Файл {tempfile} длиной в {len(text)} символ(-ов) (сохранен в {mp3}) обработан в звук за: {end_time - start_time:.2f} с.")
                            sound = AudioSegment.from_wav(wav)
                            sound.export(mp3, format="wav")
                        print(Style.RESET_ALL)
        else:
